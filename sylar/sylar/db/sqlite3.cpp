...
int SQLite3Stmt::bind(int idx, const char* value, Type type) {
    return sqlite3_bind_text(m_stmt, idx, value, strlen(value)
            // SQLITE_STATIC：表示你传入的字符串是 静态的或永久有效的，SQLite 不会复制它，而是直接引用。
            // 要求你传入的 value 在语句执行前不会被销毁或修改。
            // SQLITE_TRANSIENT：表示 SQLite 应该 复制你传入的字符串内容，这样即使你之后释放或修改 value，它也不受影响。
            // 安全，但有性能开销。
